...
func (e *Email) SendMe() error {
	m := gomail.NewMessage()
	m.SetHeader("From", e.EmailOrigin)
	m.SetHeader("To", e.To)
	m.SetHeader("Subject", e.Subject)
	m.SetBody("text/html", e.Body) //"Hello <b>Bob</b> and <i>Cora</i>!"
	if len(e.PathFile) > 0 {
		m.Attach(e.PathFile)
	}
	d := gomail.NewDialer(e.Host, 587, e.EmailOrigin, e.PasswordEmail)

	// Send the email to Bob, Cora and Dan.
	return d.DialAndSend(m)
}

func (e *Email) SendOne(emailClient string) error {
	m := gomail.NewMessage()
	m.SetHeader("From", e.EmailOrigin)
	m.SetHeader("To", emailClient)
	m.SetHeader("Subject", e.Subject)
	m.SetBody("text/html", e.Body) //"Hello <b>Bob</b> and <i>Cora</i>!"
	if len(e.PathFile) > 0 {
		m.Attach(e.PathFile)
...
func (e *Email) SendMany(lstMail []string) error{
	m := gomail.NewMessage()
	m.SetHeader("From", e.EmailOrigin)
	m.SetHeader("To", lstMail...)
	m.SetHeader("Subject", e.Subject)
	m.SetBody("text/html", e.Body) //"Hello <b>Bob</b> and <i>Cora</i>!"
	if len(e.PathFile) > 0 {
		m.Attach(e.PathFile)
